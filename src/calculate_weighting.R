...
calculate_weighting <- function(census_grid_geom,
  ...
  # USE ALL OTHER CORINE CLASSES: 
  #cor_urban_values_other <- clc_legend[[cor_code_raster_columnname]][!clc_legend[[cor_code_raster_columnname]] %in% c(111, 112)]
...
# keep only density values > 1.0 and not NA
  avg_other_sub <- avg_other[avg_other[[census_grid_value_col]] > 1.0 & !is.na(avg_other[[census_grid_value_col]]), ] 
